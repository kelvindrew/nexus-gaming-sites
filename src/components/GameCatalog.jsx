              ...
              return (
                ...
                >
                  ...
                  <div className="p-3 sm:p-4 pt-0 space-y-1.5 sm:space-y-2">
                    ...
                    {/* Primary WhatsApp CTA + Pack Toggle */}
                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={() => onQuickWhatsAppRequest(game)}
                        className="flex-1 py-2 sm:py-2.5 px-2.5 rounded-xl btn-whatsapp-modern text-[11px] sm:text-xs font-bold shadow flex items-center justify-center gap-1.5 active:scale-95 transition-transform truncate"
                      >
                      ...
                      {onAddToCart && (
                        <button
                          ...
                          className={`p-2 sm:p-2.5 rounded-xl border text-[11px] font-bold flex items-center justify-center transition-all shrink-0 ${
                            cart.some(g => g.id === game.id)
                              ? 'bg-emerald-500/20 text-emerald-300 border-emerald-400/50 shadow-sm'
                              : 'bg-white/[0.04] hover:bg-white/[0.1] text-slate-300 border-white/[0.08] hover:text-white'
                          }`}
                          title={cart.some(g => g.id === game.id) ? "Dans le pack" : "Ajouter au pack multi-jeux"}
                        >
                          <Plus className={`w-3.5 h-3.5 ${cart.some(g => g.id === game.id) ? 'rotate-45 text-rose-400' : ''}`} />
                        </button>
                      )}
                    </div>
